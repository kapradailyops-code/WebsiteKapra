...
export function GhostCursor({
    ...
    const animate = () => {
      ...
      if (pointerActiveRef.current) {
        velocityRef.current.set(
          currentMouseRef.current.x - nextMaterial.uniforms.iMouse.value.x,
          currentMouseRef.current.y - nextMaterial.uniforms.iMouse.value.y
        );
        nextMaterial.uniforms.iMouse.value.copy(currentMouseRef.current);
        fadeOpacityRef.current = 1;
      } else {
        velocityRef.current.multiplyScalar(inertia);

        if (velocityRef.current.lengthSq() > 1e-6) {
          nextMaterial.uniforms.iMouse.value.add(velocityRef.current);
        }

        const delta = now - lastMoveTimeRef.current;

        if (delta > fadeDelay) {
          const progress = Math.min(1, (delta - fadeDelay) / fadeDuration);
          fadeOpacityRef.current = Math.max(0, 1 - progress);
        }
      }

      const trailLengthValue = trailBufRef.current.length;
      headRef.current = (headRef.current + 1) % trailLengthValue;
      trailBufRef.current[headRef.current].copy(nextMaterial.uniforms.iMouse.value);

      const trailUniform = nextMaterial.uniforms.iPrevMouse.value as THREE.Vector2[];

      for (let index = 0; index < trailLengthValue; index += 1) {
        const srcIndex = (headRef.current - index + trailLengthValue) % trailLengthValue;
        trailUniform[index].copy(trailBufRef.current[srcIndex]);
      }

      nextMaterial.uniforms.iOpacity.value = fadeOpacityRef.current;
      nextMaterial.uniforms.iTime.value = time;

      if (filmPassRef.current?.uniforms?.iTime) {
        filmPassRef.current.uniforms.iTime.value = time;
      }
  ...
  useEffect(() => {
    ...
    if (material) {
      ...
      (material.uniforms.iBaseColor.value as THREE.Vector3).set(nextColor.r, nextColor.g, nextColor.b);
    }
  }, [color]);
  ...
  const mergedStyle = useMemo<React.CSSProperties>(() => ({ zIndex, ...style }), [style, zIndex]);
